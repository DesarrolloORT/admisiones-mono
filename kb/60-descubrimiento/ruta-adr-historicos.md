...
**Decisión de ritmo (2026-09-15, confirmada por el usuario) — REVERTIDA el mismo día**: se probó en `LOTE-06`/`LOTE-07` un filtro económico por título únicamente (sin abrir diffs salvo en lo evidentemente disparador). El usuario pidió frenar ese ritmo explícitamente ("no vayamos tan rápido, no perdamos contexto, si sale un poco más caro igual tomemos el tiempo de hacerlo"): **se prioriza no perder contexto por sobre la velocidad**, aunque cueste más tiempo/tokens. A partir de aquí:
- Se vuelve a revisar cada commit con más profundidad que un simple título (cuerpo completo del mensaje como mínimo, diff cuando el mensaje no alcanza para descartarlo con confianza).
- `LOTE-07` (junio completo) se marca para **re-revisión** con este estándar antes de avanzar a julio, porque se hizo solo con filtro de título.
- Los lotes pueden ser más chicos en rango de fechas si el volumen del mes lo justifica, para no perder precisión.
