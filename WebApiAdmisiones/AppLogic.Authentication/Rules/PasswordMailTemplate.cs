...
public static class PasswordMailTemplate
{
    ...
    public static string BuildActivationMail(Persona person, string link)
    {
        var name = HtmlEncoder.Default.Encode(person.PrimerNombre?.Trim() ?? "usuario");
        var safeLink = HtmlEncoder.Default.Encode(link);

        return BuildInstitutionalMailBody(
            "Solicitud de numero de usuario y contrase&ntilde;a",
            $"Estimado/a {name}:",
            new[]
            {
                $"Tu registro se ha realizado con exito. Para crear tu contrase&ntilde;a e ingresar al sitio, hac&eacute; clic en el siguiente enlace:<br><br>{BuildLinkHtml(safeLink, "Crear contrase&ntilde;a")}",
    ...
    public static string BuildRecoveryMail(Persona person, string link)
    {
        var name = HtmlEncoder.Default.Encode(person.PrimerNombre?.Trim() ?? "usuario");
        var safeLink = HtmlEncoder.Default.Encode(link);
        ...
        return BuildInstitutionalMailBody(
            ...
            new[]
            {
                $"Recibimos una solicitud para recuperar tu contrase&ntilde;a de Admisiones. Para crear una nueva contrase&ntilde;a e ingresar al sitio, hac&eacute; clic en el siguiente enlace:<br><br>{BuildLinkHtml(safeLink, "Recuperar contrase&ntilde;a")}",
                "Por seguridad, el link vence en el plazo indicado por el sistema y puede usarse una sola vez.",
    ...
    /// </summary>
    public static string BuildActivationMailWithoutPerson(string link)
    {
        var safeLink = HtmlEncoder.Default.Encode(link);
    ...
    private static string BuildInstitutionalMailBody(
        string titulo,
        string destinatario,
    ...
    private static string BuildLinkHtml(string safeLink, string texto)
    {
