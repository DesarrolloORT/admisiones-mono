...
public class FondoDeBecaService(IUnitOfWorkFactory uowFactory) : IFondoDeBecaServices
{
    ...
    public OperationResult<IEnumerable<DtoEmpresaDevart>> ObtenerUniversidades(long codigoPais)
    {
        ...
        if (pais == null)
            return OperationResult<IEnumerable<DtoEmpresaDevart>>.IsFailed(
                "FDB_UV_01", nameof(ObtenerUniversidades), "El país indicado es inválido.", 400);
    ...
    public OperationResult<DtoArchivoDescarga> DescargarArchivoIngreso(long codigoPersona, long idIngresoMensualNF)
    {
        ...
        if (!ingresoResult.Success || ingresoResult.Data is null)
            return ingresoResult.Failure().As<DtoArchivoDescarga>();

        var ingreso = ingresoResult.Data;
        var archivo = ingreso.ArchivoIngresoNfDj;
        if (archivo is null || archivo.Length == 0)
            return OperationResult<DtoArchivoDescarga>.IsFailed(
                "FDB_DAI_05", nameof(DescargarArchivoIngreso), "El ingreso mensual indicado no tiene archivo adjunto.", 404);
    ...
    public OperationResult<DtoArchivoDescarga> DescargarArchivoEgreso(long codigoPersona, long idEgresoMensualNF)
    {
        ...
        if (!egresoResult.Success || egresoResult.Data is null)
            return egresoResult.Failure().As<DtoArchivoDescarga>();

        var egreso = egresoResult.Data;
        var archivo = egreso.ArchivoEgresoMensualNfDj;
        if (archivo is null || archivo.Length == 0)
            return OperationResult<DtoArchivoDescarga>.IsFailed(
                "FDB_DAE_03", nameof(DescargarArchivoEgreso), "El egreso mensual indicado no tiene archivo adjunto.", 404);
    ...
    public OperationResult<bool> SubirArchivoRevalidaDJ(long codigoPersona, long idDeclaracionJuradaWeb, byte[] fileContent, string fileName)
    {
        ...
        using var uow = _uowFactory.Create();
        var declaracionResult = ObtenerDeclaracionAutorizada(uow, codigoPersona, idDeclaracionJuradaWeb, nameof(SubirArchivoRevalidaDJ), "FDB_SAR");
        if (!declaracionResult.Success || declaracionResult.Data is null)
            return declaracionResult.Failure().As<bool>();
    ...
    {
        using var uow = _uowFactory.Create();
        var declaracionResult = ObtenerDeclaracionAutorizada(uow, codigoPersona, idDeclaracionJuradaWeb, nameof(DescargarArchivoRevalidaDJ), "FDB_DAR");
        if (!declaracionResult.Success || declaracionResult.Data is null)
            return declaracionResult.Failure().As<DtoArchivoDescarga>();

        var declaracion = declaracionResult.Data;
        if (declaracion.PdfFormRevalidasDj is null || declaracion.PdfFormRevalidasDj.Length == 0)
            return OperationResult<DtoArchivoDescarga>.IsFailed(
                "FDB_DAR_03", nameof(DescargarArchivoRevalidaDJ), "La declaración jurada indicada no tiene archivo de reválida adjunto.", 404);

        var extension = NormalizarExtension(declaracion.ExtensionPdfRevalidasDj);
        var nombreArchivo = ConstruirNombreArchivo(declaracion.NombrePdfRevalidasDj, extension, $"revalida_{idDeclaracionJuradaWeb}");
    ...
    {
        using var uow = _uowFactory.Create();
        var declaracionResult = ObtenerDeclaracionAutorizada(uow, codigoPersona, idDeclaracionJuradaWeb, nameof(EliminarArchivoRevalidaDJ), "FDB_EAR");
        if (!declaracionResult.Success || declaracionResult.Data is null)
            return declaracionResult.Failure().As<bool>();
