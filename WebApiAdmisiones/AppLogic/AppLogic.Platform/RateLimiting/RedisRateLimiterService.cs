...
public class RedisRateLimiterService(
    ...
    ILogger<RedisRateLimiterService> logger) : IRateLimiterService
{
    private readonly IConnectionMultiplexer _redis = redis;
    private readonly ILogger<RedisRateLimiterService> _logger = logger;
    ...
    public async Task<RateLimitValidationResult> ValidateAsync(
        ...
        TimeSpan window)
    {
        var normalizedDoc = TextNormalization.NormalizeDocumentForKey(document);
        var key = $"login-account:{documentType}:{normalizedDoc}:ip:{ipAddress}";
