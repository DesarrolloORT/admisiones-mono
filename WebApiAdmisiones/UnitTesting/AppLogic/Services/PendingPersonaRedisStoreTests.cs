...
{
    public class PendingPersonaRedisStoreTests
    {
        private static DtoRegistroPendingPersona CrearPending(string flowId, string tipoDocumento = "CI", string documento = "1234567-2")
            => new()
            {
                FlowId = flowId,
        ...
        private static PendingPersonaRedisStore CrearStore(Dictionary<string, string> redis)
        {
            ...
            return new PendingPersonaRedisStore(connectionMock.Object);
        }
        ...
        public async Task SaveAsync_AlsoWritesDocumentoToFlowIdMapping()
        {
            var redis = new Dictionary<string, string>();
            var store = CrearStore(redis);
            var pending = CrearPending("flow-1", tipoDocumento: "CI", documento: "1234567-2");
        ...
        public async Task ResolverFlowIdPorDocumentoAsync_WhenNoMappingExists_ReturnsNull()
        {
            ...
            var result = await store.ResolverFlowIdPorDocumentoAsync("CI", "1234567-2");
