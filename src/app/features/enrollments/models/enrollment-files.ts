...
import type {
  EnrollmentIdentityDocumentFile,
  EnrollmentIdentityUploadFile,
...
export async function toIdentityUploadFile(file: File): Promise<EnrollmentIdentityUploadFile> {
  ...
  return {
    fileName: uploadFileName(prepared, mimeType),
    content: await readFileAsBase64(prepared),
  };
}

export function toIdentityFile(
  file: EnrollmentIdentityDocumentFile | null | undefined,
  baseName: string
): File | null {
  const rawContent = file?.content?.trim();
  if (!rawContent) return null;

  const providedName = safeFileName(file?.fileName, '');
