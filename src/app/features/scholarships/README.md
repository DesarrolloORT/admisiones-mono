...
[inscripciones](../enrollments/README.md). **Antes de tocar nada, leé ese
...
| -------------------- | --------------------------------------------------- | ----------------------------------------------------------- |
| Definición de pasos  | `models/enrollment-process.ts` (`ENROLLMENT_STEPS`) | `models/scholarship-process.ts` (`SCHOLARSHIP_STEPS`) ✅    |
| Motor de pasos       | `shared/process-flow/process-flow.ts` (compartido)  | el mismo ✅                                                 |
| Store del proceso    | `store/enrollment-process.ts`                       | `store/scholarship-process.ts` ✅                           |
| Fachada orquestadora | `facades/enrollment-process.ts`                     | `facades/scholarship-process.ts` ✅                         |
| Fachadas de sección  | `proposal` / `survey` / `payment`                   | `facades/scholarship-proposal.ts` ✅ (resto ⬜)             |
| Store de formularios | `store/enrollment-forms.ts`                         | `store/scholarship-forms.ts` ✅ (solo form académico)       |
| Services + endpoints | `services/` + `endpoints/`                          | _pendiente_ ⬜                                              |
| Resolvers            | `resolvers/`                                        | _pendiente_ ⬜                                              |
| Persistencia (draft) | `services/enrollment-resume-context.ts`             | _pendiente_ ⬜                                              |
| Página contenedora   | `pages/layout`                                      | `pages/fbr` ✅ (ya usa el flow)                             |
| Componentes de paso  | `pages/steps/enrollment-*-step`                     | `components/scholarship-*-step` ✅ (paso 1 con avance real) |
...
- **`models/scholarship-process.ts`** — `ScholarshipStep` + `SCHOLARSHIP_STEPS`
  (`application-info` → `personal-info` → `confirmation`). Editá este array para
  agregar/quitar/reordenar pasos.
- **`store/scholarship-process.ts`** — `ScholarshipProcessStore`, envuelve
  `createProcessFlow(SCHOLARSHIP_STEPS, 'application-info')`. Fuente de verdad del
  paso actual.
...
- **Paso 1 funcionando end-to-end (sin API):**
  ...
  - `facades/scholarship-proposal.ts` (`ScholarshipProposalFacade`) — espejo de
    `EnrollmentProposalFacade`: expone el form y el `AcademicProposalSelection`,
  ...
  - `components/scholarship-academic-step` — reusa el `app-academic-proposal-select`
    de catalogs (igual que `enrollment-academic-step`); su botón **Continuar**
...
3. **Formularios.** `ScholarshipFormsStore` ya tiene el form del paso 1. Agregá
   ahí los `FormGroup` de los pasos siguientes, igual que `EnrollmentFormsStore`.
...
5. **Step de confirmación.** Creá `components/scholarship-confirmation-step` y
   montalo en el `@case ('confirmation')` de `fbr.html` (hoy es un punto de
   extensión vacío).
6. **(Opcional) Persistencia y resolvers**, si el flujo necesita retomarse, igual
   que `enrollment-resume-context` y los resolvers de inscripciones.
