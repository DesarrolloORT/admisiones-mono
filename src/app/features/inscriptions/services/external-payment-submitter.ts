...
export class ExternalPaymentSubmitter {
  ...
  public submit(request: ExternalPaymentRequest): boolean {
    const url = toHttpUrl(request.urlPago);
    if (!url || !request.parametrosEncriptados.trim()) return false;
    ...
    input.value = JSON.stringify({
      params: `parametrosEncriptados=${request.parametrosEncriptados}`,
    });
