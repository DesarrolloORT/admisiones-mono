...
export function resolveToolkitSignals(root) {
  ...
  if (
    preCommitPatterns.includes("package.json") ||
    preCommitPatterns.includes("./package.json")
  ) {
  ...
  return {
    ...
    designSystemDocs,
    figmaMcpServer,
    figmaMcpServerAvailable:
      figmaMcpServer !== null && workspaceMcpServers.has(figmaMcpServer),
...
export function classifyToolGuardrail(input) {
  ...
  const hasSensitiveTargets = isSensitiveToolTarget(joinedTargets) || isSensitiveToolTarget(command);
...
export function getWorkspaceHash(root) {
  return createHash("sha256").update(normalizePath(resolve(root))).digest("hex").slice(0, 12);
}
...
export function buildRepoSummary(root) {
  ...
  const profiles =
    Array.isArray(manifest?.profiles) && manifest.profiles.length > 0
      ? uniq(manifest.profiles.map((profile) => normalizeProfileName(profile))).slice(0, 4).join(", ")
      : "custom";
...
export function evaluatePreToolUse(state, input) {
  ...
  return {
    ...
    shouldAsk:
      (kind === "edit" && targets.length > thresholds.wideEdit) || warningLevel === "ask",
    searchPreview,
    warningLevel,
    warningOccurrence:
      kind === "search" && searchPreview ? searchPreview.previousCount + 1 : 0,
  };
}
...
export function buildPreToolResponse(state, evaluation, guardrailContext = "") {
  ...
  if (evaluation.kind === "edit" && evaluation.targets.length > 0) {
    hints.push("Mantiene el cambio dentro del scope activo y evita editar mas archivos de los necesarios.");
  }
  ...
  const permissionDecisionReason = evaluation.shouldAsk
    ? buildWarningReason(evaluation) || `Operacion amplia detectada: ${evaluation.reasons.join(", ")}.`
