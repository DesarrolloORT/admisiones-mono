...
3. El usuario abre el link y el frontend llama a `POST auth/activate-password-link` enviando `{ "token": "..." }`.
4. Si la respuesta es exitosa, la API setea la cookie temporal `X-Password-Activation`.
5. El frontend muestra el formulario de nueva password y llama a `POST auth/complete-initial-password` enviando `{ "passwordNueva": "..." }`.
